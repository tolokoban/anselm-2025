...
export class Logic extends TgdPainterLogic {
    ...
    private readonly mark: PainterMark
    ...
    constructor(
        ...
        this.mark = new PainterMark(context)
    ...
    private readonly update = (time: number, delta: number) => {
            ...
            if (isNumber(anglePad)) {
                ...
                if (hit) {
                    ...
                    if (
                        ...
                        let index = 0
                        const cx = Math.sin(hit.normalAngle)
                        const cy = -Math.cos(hit.normalAngle)
                        if (cx > 0.1) {
                            index = 1
                        } else if (cx < -0.1) {
                            index = 3
                        ...
                        if (isDebugMode()) this.mark.add(ball.x, ball.y, index)
        ...
        if (isDebugMode()) this.mark.paint(time, delta)
    }
